 ...
 * simuredJava.java
 *
 * Created on 6 de julio de 2004, 9:11
 */

/**
 *
 * @author  Oscar and Fernando
 * For Applet version search APPLET in the code (only one occurrence).
 *
 */
//import Main.GraficoPkg.TRCFilter;
import Red.Channel;
import Red.Device;
import Red.Red;
import java.awt.*;
...
//public class Jsimured extends javax.swing.JApplet{
  ...
  public Jsimured(boolean esAppl, boolean esComan) {
    ...
    if (esComando)
    {
        ...
        // FER creo que aquí debería esperar a que acabe el thread y luego salir del sistema
    ...
    setIdioma(Idioma());
    SIMUVAR2=0; // Había que inicializarlo
...
//#endif
   ...
   */
    ...
    private void initComponents() {
        ...
        AdelantoCheck.setBounds(60, 90, 200, 25);
        ...
        EmisionTxt.setBounds(300, 40, 80, 20);
        ...
        jPanel16.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Escala", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11))); // NOI18N
        ...
        jButton3.setBounds(100, 270, 300, 25);
        ...
        jButton6.setBounds(270, 160, 230, 23);
        ...
        jLabel19.setBounds(10, 300, 40, 14);
        ...
        jLabel23.setBounds(30, 70, 300, 14);
...
// TODO add your handling code here:
    ...
    public static void setIdioma(int elidioma)
    {
      ...
      if (elidioma==1){
        ...
        RoutingTxt.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Orden de Dimension (XY) en mallas (determinista)", "Orden de Dimension (XY) en toros (tiene bloqueos mortales)", "Duato basado en XY para mallas (adaptativo)", "Completamente Adaptativo para mallas (tiene bloqueos)"}));
...
public static Runnable Simular = new Runnable() {   
  ...
  //private static final int MAX_TH_NUM=10;
    ...
    //Jsimured.SimuControl.Write(0); // podría ser 1, mostrar o paso a paso
    ...
    // en linea de comandos esto se debería mandar a la salida estandar...
    if (esComando)
    {
        printCSV=System.out;
    }
    ...
    // calcula el número de threads optimo
...
//       Muestra("Total Packet Latency:               "+FloatToStr((double)(LaRedGlobal->LatTotal[set])/realTrans));
    ...
    public static synchronized void Dibuja(Red LaRed){
      ...
      // igual es más apropiado el BufferedImage en vez de Image
      ...
      /** Nos creamos 3 imágenes para dibujar el nodo, el nodo con cambios,
       * y la imagen diferencia
       */
      
      // He puesto ese componente pero igual se puede poner cualquier otro, lo importante es poner uno estático
...
//        AreaDibujo.paintComponents(ImgDibujo);
    ...
    {   
        ...
        for (int i=0; i < sims.size(); i++){
            ...
            {
                if (i==0) return;
                else continue; // gráfica con menos de dos puntos
            }
